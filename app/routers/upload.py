...
async def upload_las(file: UploadFile = File(...), downsample: int = Query(5, ge=1, le=100)) -> dict:
    ...
    try:
        ...
        for k,v in list(petro.items()):
            if isinstance(v,list): petro[k]=v[::downsample]
        return {"status":"success","metadata":parsed['metadata'],"petrophysics":petro,"zones":zonas,"evaporite_flags":evap,"qc_flags":qc}
    finally:
        os.unlink(ruta)
